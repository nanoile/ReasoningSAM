...
class InstructSAM:
    def __init__(self, annotations=None, img_dir=None, count_dir=None, rp_preds=None, device="cuda:0"):
        ...
        if annotations is not None:
            ...
            self.category_name_to_id = {cat['name']: cat['id'] for cat in self.anns['categories']}
            self.img_name_to_id = {img['file_name']: img['id'] for img in self.anns['images']}
        else:
            self.anns = None
    ...
    def load_rps_and_cnts(self, thr=0.0):
        if self.count_dir is not None:
            ...
            img_cnt_path = os.path.join(self.count_dir, self.img_name.replace(img_suffix, '.json'))
            try:
                with open(img_cnt_path, 'r') as f:
                    object_cnt = json.load(f)
                self.pred_counts = {k: v for k, v in object_cnt.items() if v != 0}
            except:
                print(f"Error loading {img_cnt_path}, check if the file exists")
                self.pred_counts = {}
    ...
    def segment_anything(self, mask_generator, max_masks=200, min_mask_region_area=0, rle=True):
        ...
        if rle:
            self.segmentations = [self.boolean_mask_to_rle(mask['segmentation']) for mask in masks]
        else:
    ...
    def calculate_vocab_text_features(self, clip_model, tokenizer):
        ...
        with torch.no_grad():
            if isinstance(clip_model, open_clip.model.CLIP):
                ...
                TARGET_FEATURES = clip_model.get_text_features(TARGET_CATEGORY_tokens)
    ...
    def calculate_pred_text_features(self, clip_model, tokenizer, use_vocab=False):
        ...
        Args:
            ...
            use_vocab (bool, optional): _description_. Defaults to False.
        ...
        if use_vocab:
            ...
            text_tokens = tokenizer(cat_prompt).to(next(clip_model.parameters()).device)
            with torch.no_grad():
                if isinstance(clip_model, open_clip.model.CLIP):
    ...
    def match_boxes_and_labels(self,
        ...
        boxes_final, labels_final, segmentations_final, scores_final = match_boxes_and_counts(
            ...
            crop_scale=crop_scale, batch_size=batch_size, full_categories=full_categories,
